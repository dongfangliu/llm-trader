...
useSeoMeta({
  title: () => `${first.value?.symbol_name || symbol.value} 模型复盘历史记录`,
  description: () => `${first.value?.symbol_name || symbol.value} 的已结算模型复盘历史，展示历史方向、实际涨跌和命中情况。`,
})
</script>
...
<template>
  <main class="seo-page">
    <header class="hero">
      ...
      <p>已结算模型复盘记录。页面只展示历史结果，不提供未结算预测。</p>
    </header>
    <section class="list">
      <NuxtLink
