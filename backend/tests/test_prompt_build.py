"""_build_enhanced_prompt 渲染回归测试（不调 LLM，只验证拼出的 prompt 文本）。"""

import numpy as np
import pandas as pd

from src.services.data.trend_features import compute_trend_features
from src.services.llm.llm_service import _build_enhanced_prompt
...
def test_daily_prompt_has_methodology_and_no_emoji():
    ...
    p = _build_enhanced_prompt(_df(closes), symbol="600519", market="a", period="daily")
    assert "趋势类型(时钟方向)" in p
    assert "多头排列" in p
    assert "抵扣价" in p
...
def test_futures_prompt_keeps_action_enum():
    closes = 100 * np.exp(-0.001 * np.arange(300))
    p = _build_enhanced_prompt(_df(closes), symbol="rb2410", market="futures", period="daily")
